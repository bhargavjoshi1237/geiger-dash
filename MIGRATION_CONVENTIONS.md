...
> **This product's schema is `public`.** Its migration ledger is
> `public.geiger_migrations`. Substitute `public` wherever this doc writes
> `<schema>` — the authoritative value is `schema` in `geiger-orm.config.mjs`.
>
> **Dash is the exception to "one schema per product".** It is the suite hub and
> owns the genuinely cross-product tables (`public.projects`, org, billing,
> OAuth), so its schema *is* `public`. Where this doc says "never put product
> tables in `public`", that rule is aimed at the product apps — for Dash it is
> the job, and the discipline is instead to keep anything Dash-only out of the
> other products' way. Dash also carries a **Prisma** schema for part of its
> data: Prisma-managed tables are migrated by Prisma (`npm run db:migrate`),
> `@geiger/orm` owns the rest, and neither may write the other's tables.
...
`DIRECT_URL` is **never** `NEXT_PUBLIC_`. It is a superuser-grade credential and
...
-- Add seat maps
--
-- Owns public.seat_maps. Free-form header: what this changes and why.
...
alter table public.tickets add column if not exists tier text;   -- back-fill older copies
...
drop policy if exists tickets_demo_all on public.tickets;
create policy tickets_demo_all on public.tickets for all to anon, authenticated
...
-- @no-transaction
-- @up
create index concurrently if not exists tickets_event_idx on public.tickets (event_id);
```
...
| Put *product-app* tables in `public` | Dash owns `public`; product apps own their own schema |
| `references public.users(id)` | Plain `created_by uuid` — that table does not exist here |
| Expose `DIRECT_URL` to the client | Server-only, migrations only |
...
| `Could not connect` | `DIRECT_URL` missing or wrong. The CLI reads `.env` then `.env.local` from the project root. |
...
- [ ] Every object is `public`-qualified; no `flow_` prefix; not a Prisma-owned table
