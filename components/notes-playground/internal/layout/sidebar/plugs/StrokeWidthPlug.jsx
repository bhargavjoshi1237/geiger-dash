  ...
  return (
    <Popover>
      ...
      <PopoverContent
        ...
        align="start"
        sideOffset={8}
        className="w-40 bg-[#1e1e1e] border-border p-1 shadow-xl rounded-xl"
      >
        <div className="flex flex-col gap-0.5">
          <Label className="px-2 py-1.5 text-[10px] uppercase tracking-wider text-foreground0 font-semibold">
            Thickness
          </Label>
          {STROKE_WIDTHS.map((sw) => (
            <button
              ...
              className={`flex items-center gap-3 px-2 py-2 rounded-lg text-sm transition-colors
                ${value === sw.value ? "bg-surface-hover text-white" : "text-muted-foreground hover:bg-surface-hover/50 hover:text-foreground"}
            `}
            >
              <div
