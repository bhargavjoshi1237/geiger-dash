...
export default function MembersTab({
  ...
  return (
    <div className="h-full flex flex-col">
      {!isSessionActive ? (
        <div className="flex flex-col items-center justify-center h-[200px] text-foreground0 space-y-2">
        ...
        <div className="flex-1 -mx-2">
          <ScrollArea className="h-[300px] px-2">
            {isHost && requestedMembers.length > 0 && (
              <div className="mb-6">
                <h4 className="text-xs font-semibold text-foreground0 uppercase tracking-wider mb-2 px-1 flex items-center justify-between">
                  Requests
                  <Badge
                    variant="secondary"
            ...
            <div>
              <h4 className="text-xs font-semibold text-foreground0 uppercase tracking-wider mb-2 px-1">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        <div
                          className="w-8 h-8 rounded-full bg-surface-hover flex items-center justify-center text-xs font-medium text-muted-foreground border border-border-strong"
                          style={
                            user.color
                              ? {
                      ...
                      <div>
                        <div className="text-sm font-medium text-foreground flex items-center gap-2">
                          {user.name}
                          {user.isMe && (
                            <span className="text-foreground0 text-xs">(You)</span>
                          )}
                          {user.role === "Owner" && (
                            <Badge
                              variant="outline"
                              className="text-[10px] h-4 py-0 px-1 border-border-strong text-muted-foreground font-normal"
                            >
                              Host
                            </Badge>
                          )}
