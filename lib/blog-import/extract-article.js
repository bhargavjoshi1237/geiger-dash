...
// HTML page and return the readable article text — plus the image links found
// in the page so the AI import can keep them in the generated content.
...
// Pick the most meaningful src from an <img> tag, preferring lazy-load
// attributes and srcset over tiny inline placeholders.
function pickImgSrc(tag) {
  for (const attr of ['src', 'data-src', 'data-original', 'data-lazy-src']) {
    const value = readAttr(tag, attr)
    if (value && !/^data:image\/(gif|svg)/i.test(value)) return value
  }
...
// Resolve an image reference to something usable: keep absolute http(s) URLs and
// inline data URIs, resolve relative paths against a base URL when we have one,
// and drop anything we can't turn into an absolute reference.
...
// Collect the image links from an HTML page, de-duped and in document order.
export function extractArticleImages(html, { baseUrl = '', max = 20 } = {}) {
  const source = String(html || '')
  if (!source.trim()) return []
  ...
  while ((match = imgTag.exec(source)) !== null) {
    const tag = match[0]
    const resolved = resolveImageUrl(pickImgSrc(tag), baseUrl)
    if (!resolved || seen.has(resolved)) continue
    seen.add(resolved)
    images.push({ src: resolved, alt: readAttr(tag, 'alt') })
...
// Fetch a remote page and return its extracted article text plus image links.
export async function fetchArticle(url, { maxLines = 400 } = {}) {
  ...
  const images = extractArticleImages(body, { baseUrl: response.url || target })
  return { text, images }
}
