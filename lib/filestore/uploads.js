...
import { resolveReadUrl } from "./drivers/index.js";
...
async function recordUploadError({
  stage, code, error, namespaceId, providerId, nodeId, uploadId, size, mimeType,
}) {
  const message = error?.message || String(error);
  const detail = { stage, code, message, uploadId, sizeBytes: size, mimeType };
  const context = { ...detail, namespaceId, providerId, nodeId };
  if (error instanceof Error) console.error("[filestore.upload]", context, error);
  else console.error("[filestore.upload]", context);
  await logEvent({ type: "error", namespaceId, providerId, nodeId, bytes: size || 0, detail });
}
...
export async function beginUpload({
  ...
  for (let attempt = 0; attempt < MAX_PLACEMENT_ATTEMPTS; attempt += 1) {
    ...
    if (!resolved.provider) {
      await recordUploadError({
        stage: "placement", code: resolved.code, error: resolved.error,
        namespaceId: namespace.id, size, mimeType,
      });
  ...
  if (!providerRow) {
    await recordUploadError({
      stage: "reservation", code: "pool_full", error: "Could not reserve space on any eligible provider.",
      namespaceId: namespace.id, size, mimeType,
    });
    return {
      ok: false,
  ...
  try {
    ...
    await recordUploadError({
      stage: "node", code: "invalid_path", error: err,
      namespaceId: namespace.id, providerId: providerRow.id, size, mimeType,
    });
  ...
  try {
    ...
    await supabase
      ...
      .eq("id", node.id);
    await recordUploadError({
      stage: "ticket", code: "driver_error", error: err,
      namespaceId: namespace.id, providerId: providerRow.id, nodeId: node.id, size, mimeType,
    });
  ...
  if (placementError) {
    await releaseBytes(providerRow.id, size);
    await recordUploadError({
      stage: "placement_record", code: "placement_error", error: placementError,
      namespaceId: namespace.id, providerId: providerRow.id, nodeId: node.id, size, mimeType,
    });
  ...
  if (uploadError) {
    await releaseBytes(providerRow.id, size);
    await recordUploadError({
      stage: "upload_record", code: "ticket_error", error: uploadError,
      namespaceId: namespace.id, providerId: providerRow.id, nodeId: node.id, size, mimeType,
    });
...
export async function commitUpload({
  ...
  try {
    ...
    await failUpload(upload, providerRow, err, "commit");
    return { ok: false, code: "driver_error", error: err.message };
  }
...
async function failUpload(upload, providerRow, error, stage) {
  const supabase = filestoreClient();
  ...
  await recordUploadError({
    stage, code: "driver_error", error,
    namespaceId: upload.namespace_id, providerId: upload.provider_id,
    nodeId: upload.node_id, uploadId: upload.id, size: Number(upload.reserved_bytes || 0),
  });
...
export async function transferBytes({ uploadId, body, mime }) {
  ...
  try {
    ...
    await failUpload(upload, providerRow, err, "transfer");
...
export async function migrateNode({ nodeId, targetProviderId }) {
  ...
  try {
    // Resolve afresh: providers with expiring links (signed S3, Playbook) outlive the stored URL.
    const sourceUrl = current.providers
      ? await resolveReadUrl(
          {
            driver: current.providers.driver,
            config: current.providers.config,
            secretsEnvelope: current.providers.secrets,
            capabilities: current.providers.capabilities,
            publicUrlTemplate: current.providers.public_url_template,
          },
          current.provider_key,
          current.provider_url
        )
      : current.provider_url;
