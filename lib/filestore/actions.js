...
import { getDriver, builtinCatalog } from "./drivers/index.js";
import {
  probeProvider,
  reconcileAll,
  loadProviderRow,
...
export async function saveProviderAction(input = {}) {
  ...
  try {
    if (getDriver(driver).builtin) {
      return { ok: false, error: "Built-in providers are switched on or off, not configured." };
    }
  } catch (err) {
  ...
  if (existing && getDriver(existing.driver).builtin) {
    return { ok: false, error: "Built-in providers are switched on or off, not configured." };
  }
...
// Built-in providers get one row per driver, created on first enable; enabling re-probes so only a working one takes uploads.
export async function setBuiltinProviderAction(driverId, enabled) {
  const { user, error: authError } = await requireAuth();
  if (authError) return { ok: false, error: authError };

  const entry = builtinCatalog().find((item) => item.driver === driverId);
  if (!entry) return { ok: false, error: "Unknown built-in provider." };
  ...
  // Disabling stops new uploads only; files already on the provider stay readable.
  if (!enabled) {
  ...
  if (!entry.configured) {
    return { ok: false, error: `${entry.label} is not configured on this deployment.` };
  }
  ...
  if (providerId) {
    ...
    // New built-ins fill last, so they only take uploads a rule pins to them or that spill over.
    const { data, error } = await supabase
      .from("providers")
      .insert({
        name: entry.label,
        driver: driverId,
        status: "unverified",
        priority: Number(rows?.[0]?.priority || 0) + 10,
        capabilities: getDriver(driverId).defaultCapabilities,
        capacity_bytes: getDriver(driverId).defaultCapacityBytes || 0,
