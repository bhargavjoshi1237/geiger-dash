...
//
// Two entry points share the delivery core:
//   sendTemplateEmail  — renders one of this project's stored templates
//   sendRenderedEmail  — takes HTML a caller rendered itself (geiger-docs runs
//                        Documenso's own template library and only needs our
//                        delivery + log)
...
export async function sendRenderedEmail({
  ...
  if (!to || !html || !subject) {
    return { ok: false, error: "`to`, `html` and `subject` are all required." };
  }

  return deliver({ to, from, subject, html, templateKey, data: {}, project, apiKeyId });
}

async function deliver({ to, from, subject, html, templateKey, data, project, apiKeyId }) {
