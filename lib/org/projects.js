...
export async function getOrganizationProjects(organizationId) {
  ...
  const { data: organization, error: organizationError } = await supabase
    .from("organizations")
    .select("id, name, description, created_at, is_active, country, phone, metadata, avatar_url")
  ...
  return {
    ...
    projects: (data || []).map((row) => {
      ...
      return {
        ...
        // Display name source of truth for both the dash and Flow is
        // projects.name; the plan title is a legacy fallback.
